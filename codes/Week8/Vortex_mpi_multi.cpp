...
void write_to_file(const char* filename, std::complex<double> *positions, int N, int myrank, int chunk_size);
...
int main(int argc, char** argv)
{
	...
	while(t<3){
		if(count%100==0){
			sprintf(filename,"vortices%d_%d.txt", no, myrank); // now same as "picture#nr"
			write_to_file(filename, vortices, N, myrank, chunk_size);
			no++;
		}
