...
private:
    ...
    /// index of the n'th neighbour of cell c; n = {1, ... 8}
    size_type cell_neighbor(size_type c, size_type n) const
    {
	    ...
	    // return the coloumn and row of the neighbour n according the
	    // row and coloum on the "real" field of cells
	    switch( n % 3 )
	    {
		    case  0:	break;
	    ...
	    switch( n /	3 )
 {
    ...
    /// updating cell list: add to new cell and delete from old cell
    ...
    void sort_cells (configuration& x, configuration& a, configuration& b, configuration& d, configuration& e)
    {
	    for ( size_type c=0; c < num_cells(); ++c )
	    {
		    for ( size_type i=0; i < x[c].size(); ++i )
		    {
			    ...
			    if (ci != c)
			    {
	  			    // remind: configuration is the vector conaining num_cells * vectors 
				    // each vecrot (the cells) containing arrays with the coordinates!
				    // swap(particle_position, current_cellID, particleID, new_cellID)
				    swap_cell(x,c,i,ci);
				    swap_cell(a,c,i,ci);
				    swap_cell(b,c,i,ci);
				    swap_cell(d,c,i,ci);
