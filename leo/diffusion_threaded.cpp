...
int main(int argc, char *argv[])
{
	...
	for(unsigned int thr = 0; thr < nthreads; thr++)
	{
		...
		threads[thr] = thread([&,thr]()
				...
				{
				cout << "thread "<< thr <<" running " << t <<" th time!\n";
				wait = 1;
				while(wait==1){
					      if(status[thr]=true) // if main didn't set status of thr to true it waits
							 {wait=0;}  // else it may go on (change wait to zero and escape while loop
					      }
				
				/*
				 * UNABLED FOR DEBUGGING!
	...
	cout << "Main progressing in " << t<<" th round!\n";
