...
VelocitySolverNSquared::VelocitySolverNSquared(ArrayOfParticles & dstParticles, ArrayOfParticles & srcParticles, int rank, int size, const double wingSpan)
: dstParticles(dstParticles), srcParticles(srcParticles), rank(rank), size(size), wingSpan(wingSpan), timeC(0), timeT(0)
{
}

void VelocitySolverNSquared::ComputeVelocity()
{
	const double i2pi = wingSpan*.5/M_PI;
    ...
    for (int pass=0; pass<size; pass++)
    {
        ...
        Timer timerC;
        timerC.start();
        for (int i=0; i<N; i++) // loop over dst
        {
            double u=0;
            double v=0;
            
            for (int j=0; j<N; j++) // loop over src
            {
